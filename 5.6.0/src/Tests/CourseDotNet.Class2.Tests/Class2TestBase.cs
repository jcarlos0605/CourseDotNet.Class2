...
using CourseDotNet.Class2.Migrations.SeedData;
...
namespace CourseDotNet.Class2.Tests
{
    public abstract class Class2TestBase : AbpIntegratedTestBase<Class2TestModule>
    {
        ...
        protected void UsingDbContext(Action<Class2DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Class2DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Class2DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Class2DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Class2DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Class2DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Class2DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Class2DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Class2DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Class2DbContext, Task<T>> func)
        {
            T result;
